document.addEventListener('DOMContentLoaded', () => {
  ...
  const attemptsToGuarantee = [0, 1, 2, 3, 4, 5, 10, 15, 20, 25, 30, 35, 40, 50, 60, 70];
  ...
  const updateEndLevelOptions = () => {
    const startLevel = parseInt(startLevelSelect.value);
    const endLevelOptions = endLevelSelect.options;
    let currentEndLevel = parseInt(endLevelSelect.value);
    let firstValidOption = null;

    for (let i = 0; i < endLevelOptions.length; i++) {
      const optionValue = parseInt(endLevelOptions[i].value);
      const isDisabled = optionValue <= startLevel;
      endLevelOptions[i].disabled = isDisabled;
      
      if (!isDisabled && firstValidOption === null) {
        firstValidOption = optionValue;
      }
    }

    if (currentEndLevel <= startLevel && firstValidOption !== null) {
      endLevelSelect.value = firstValidOption;
    }
  };

  const updateAttemptLimits = () => {
    const startLevel = parseInt(startLevelSelect.value);
    let maxAttempt = 0;

    if (startLevel >= 0 && startLevel < 15) {
      maxAttempt = attemptsToGuarantee[startLevel + 1] - 1;
    }

    currentAttemptInput.max = maxAttempt;
    attemptHint.textContent = `(макс: ${maxAttempt})`;

    if (parseInt(currentAttemptInput.value) > maxAttempt) {
      currentAttemptInput.value = 0;
    }
  };

  const updateSettings = () => {
    updateEndLevelOptions();
    updateAttemptLimits();
  };
  ...
  const calculateSharpening = () => {
    hideError();

    const startLevel = parseInt(startLevelSelect.value);
    const endLevel = parseInt(endLevelSelect.value);
    const currentAttempt = parseInt(currentAttemptInput.value) || 0;
    ...
    const maxAttempt = attemptsToGuarantee[startLevel + 1] - 1;
    if (currentAttempt < 0 || currentAttempt > maxAttempt) {
      showError(`Текущая попытка должна быть от 0 до ${maxAttempt}.`);
      return;
    }

    const prices = {
      cheapPart: parseInt(cheapPartPriceInput.value) || 0,
    ...
    const totalCost = 
      totalCheapParts * prices.cheapPart + 
      totalCheapTools * prices.cheapTool +
      totalStandardParts * prices.standardPart + 
      totalStandardTools * prices.standardTool +
      totalAdvancedParts * prices.advancedPart + 
      totalAdvancedTools * prices.advancedTool;

    const guaranteedCost = 
      guaranteedCheapParts * prices.cheapPart + 
      guaranteedCheapTools * prices.cheapTool +
      guaranteedStandardParts * prices.standardPart + 
      guaranteedStandardTools * prices.standardTool +
      guaranteedAdvancedParts * prices.advancedPart + 
      guaranteedAdvancedTools * prices.advancedTool;

    totalCostSpan.textContent = formatNumber(totalCost);
    guaranteedCostSpan.textContent = formatNumber(guaranteedCost);
  };

  currentAttemptInput.addEventListener('blur', () => {
    const value = parseInt(currentAttemptInput.value) || 0;
    const max = parseInt(currentAttemptInput.max) || 0;
    
    if (value < 0) {
      currentAttemptInput.value = 0;
    } else if (value > max) {
      currentAttemptInput.value = max;
    }
  });

  startLevelSelect.addEventListener('change', updateSettings);
  endLevelSelect.addEventListener('change', updateSettings);
